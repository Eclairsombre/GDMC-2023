            ...
            for z in range(0, xzDistance[1]):
                ...
                if mask.getpixel((x, z))!= 0 : #mask
                    ...
                    if delta != 0:
                        ...
                        if block.id not in lookup.LIQUIDS:
                            if delta > 0:
...
def removeTrees(treesmap, roadsArea, heightmap):
        ...
        removedTreesmap = Image.new("RGB", xzDistance, 0)
        ...
        for x in range(0, xzDistance[0]):
            for z in range(0, xzDistance[1]):
                ...
                if roadsArea.getpixel((x, z)) == 255 and treesmap.getpixel((x, z)) > 0 and (x, z) not in toRemove : #mask
                    
                    treeArea = morphology.flood(treesmap, (z, x), tolerance=1)
                    blend = Image.blend(Image.fromarray(treeArea).convert('RGB'), removedTreesmap.convert('RGB'), 0.5)
                    
                    array = np.array(blend.convert('L'))
                    bool_array = array > 1
                    removedTreesmap = Image.fromarray(bool_array)

                    toRemove.append((x, z))
                    print(x, z)

        removedTreesmap.save('data/removedTreesmap.png')

        for x in range(0, xzDistance[0]):
            for z in range(0, xzDistance[1]):
                print("removing tree in ", xzStart[0] + x, xzStart[1] + z)
                if removedTreesmap.getpixel((x, z)) != 0:
                    y = heightmap.getpixel((x, z))
                    yTree = removedTreesmap.getpixel((x, z))
